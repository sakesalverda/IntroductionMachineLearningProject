...
for i in range(3):
    ...
    sales = df[df["Company"] == i]["SalesScaled"]

    # max_sales = np.max(sales, axis = 0)
    # min_sales = np.min(sales, axis = 0)

    # sales = (sales - min_sales) / (max_sales - min_sales)

    # print(f"Company {i} has min: {min_sales} and max: {max_sales}")
    ...
    for l in range(len(seasons)):
        ...
        ax.text(dx + seasons[l]/2, max_num * 0.9, label, horizontalalignment = "center")
        ax.text(dx + seasons[l]/2, max_num * 0.8, 2020 + np.floor(l/4), horizontalalignment = "center")
        #if l % 2 == 0:
        ax.add_patch(Rectangle((dx, 0), seasons[l], max_num, alpha = 0.5, color = color))
        dx += seasons[l]
...
plt.show()
# plt.savefig('Graphs/Sales min max.png')